...
class ContextTool(BaseTool):
    ...
    def on_boot_complete(self, container):
        ...
        manifest += "2. **Plugin Template**:\n"
        manifest += "```python\n"
        manifest += "from core.base_plugin import BasePlugin\n\n"
        manifest += "class MyNewPlugin(BasePlugin):\n"
        manifest += "    def __init__(self, logger, event_bus, http, db): # Request Tools here\n"
        ...
        manifest += "        self.http.add_endpoint('/my/path', 'GET', self.my_handler)\n"
        manifest += "        self.bus.subscribe('some.event', self.execute)\n\n"
        manifest += "    def execute(self, data=None): # Core Logic\n"
        manifest += "        self.logger.info('Executing logic...')\n"
        manifest += "        return {'success': True}\n\n"
        manifest += "    def my_handler(self, data, context):\n"
