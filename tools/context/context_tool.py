...
class ContextTool(BaseTool):
    ...
    def on_boot_complete(self, container):
        ...
        manifest += "## 🏗️ Philosophy and Execution Architecture\n"
        ...
        manifest += "## 🚀 Execution and Development\n"
        ...
        manifest += "## 🛠️ How to create a new Domain & Plugin\n"
        manifest += "To extend the system, follow this pattern:\n\n"
        manifest += "1. **Create Directory**: `domains/[domain_name]/plugins/`\n"
        manifest += "2. **Plugin Template (The Core Pattern)**:\n"
        manifest += "```python\n"
        manifest += "from core.base_plugin import BasePlugin\n\n"
        manifest += "class MyNewPlugin(BasePlugin):\n"
        manifest += "    def __init__(self, logger, event_bus, http, db):\n"
        manifest += "        # Tools are injected by name automatically\n"
        manifest += "        self.logger = logger\n"
        manifest += "        self.bus = event_bus\n"
        manifest += "        self.http = http\n"
        manifest += "        self.db = db\n\n"
        manifest += "    def on_boot(self):\n"
        manifest += "        # 1. Routes & Subscriptions ONLY\n"
        manifest += "        self.http.add_endpoint('/my/path', 'GET', self.handler)\n"
        manifest += "        self.bus.subscribe('some.event', self.execute)\n\n"
        manifest += "    def execute(self, data=None): \n"
        manifest += "        # Pattern: Validate -> Process -> Act -> Respond\n"
        manifest += "        self.logger.info('Executing logic...')\n"
        manifest += "        return {'success': True, 'data': {}}\n\n"
        ...
        for name in container.list_tools():
            ...
            manifest += f"### 🔧 Tool: `{name}` (Status: {status_emoji} {health['status']})\n"
            if health["status"] != "OK":
