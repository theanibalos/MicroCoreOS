...
async def test_chain_asserted_against_real_bus_causality(event_bus):
    """The in-process flavor: a real handler chain, verified via trace history."""

    async def on_created(event: EventEnvelope):
        await event_bus.publish("order.notified", {"order_id": event.payload["id"]})

    await event_bus.subscribe("order.created", on_created)
    await event_bus.publish("order.created", {"id": 7})
    await asyncio.sleep(0.05)

    forest = build_tree(event_bus.get_trace_history())
