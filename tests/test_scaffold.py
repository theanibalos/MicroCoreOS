...
def test_new_materializes_a_bootable_project(tmp_path):
    ...
    # Auth starter: the smallest set that makes tools/auth usable — register,
    # login, and one protected endpoint. Without it auth ships with no way to
    # obtain a token.
    plugins = target / "domains" / "users" / "plugins"
    ...
    assert (target / "domains" / "users" / "migrations" / "001_create_users.sql").is_file()
